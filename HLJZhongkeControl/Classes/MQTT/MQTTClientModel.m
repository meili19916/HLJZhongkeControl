    ...
    switch (newState) {
        case MQTTSessionManagerStateConnected:
            ...
            if ([self.delegate respondsToSelector:@selector(connectStatus:)]) {
                [self.delegate connectStatus:@"连接成功"];
            }
            break;
        case MQTTSessionManagerStateConnecting:
            NSLog(@"eventCode -- 连接中");
            if ([self.delegate respondsToSelector:@selector(connectStatus:)]) {
                [self.delegate connectStatus:@"连接中..."];
            }
            break;
        case MQTTSessionManagerStateClosed:
            if ([self.delegate respondsToSelector:@selector(connectStatus:)]) {
                [self.delegate connectStatus:@"连接被关闭"];
            }
            NSLog(@"eventCode -- 连接被关闭");
            break;
        case MQTTSessionManagerStateError:
            if ([self.delegate respondsToSelector:@selector(connectStatus:)]) {
                [self.delegate connectStatus:@"连接错误"];
            }
            NSLog(@"eventCode -- 连接错误");
            break;
        case MQTTSessionManagerStateClosing:
            if ([self.delegate respondsToSelector:@selector(connectStatus:)]) {
                [self.delegate connectStatus:@"连接关闭中"];
            }
            NSLog(@"eventCode -- 关闭中");
